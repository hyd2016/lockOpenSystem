...
class LockOpenRecordActivity : AppCompatActivity() {
    private lateinit var token: String
    private lateinit var lockId: String
    private lateinit var disposable: Disposable

    companion object {
        fun startLockOpenRecordActivity(context: Context, token: String, lockId: String) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        disposable = NetworkHelper.getRetrofit().getLockOpenRecord(token, lockId)
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe(
                {
                    if (it.result == 0) {
                        lock_open_record.adapter = LockOpenRecordAdapter(it.openrecord)
                    }
